...
cmd({
  ...
  category: "convert",
  use: ".readmore Hi | I am Mughal",
  react: "📄",
  filename: __filename
}, async (conn, m, store, { args, reply }) => {
  try {
    const input = args.join(" ");
    if (!input) return reply("❌ Please provide text to split with read more.");

    const [before, after] = input.split("|").map(v => v.trim());
    const readMore = String.fromCharCode(8206).repeat(4001);
    const message = `${before || "Here"}${readMore}${after || "Continue reading..."}`;
