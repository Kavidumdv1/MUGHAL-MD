...
cmd({ 
  ...
  reply("*_MUGHAL-MD STARTED NOW...🚀_*"); 
});
//---------------------------------------------------------------------------
//  ⚠️ DO NOT MODIFY THIS FILE ⚠️  
//---------------------------------------------------------------------------
...
cmd({
  ...
  try {
    ...
    if (!isBotAdmins) return reply('*📛 𝑰 𝒏𝒆𝒆𝒅 𝒕𝒐 𝒃𝒆 𝒂𝒏 𝒂𝒅𝒎𝒊𝒏 𝒕𝒐 𝒖𝒔𝒆 𝒕𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅.*');
    if (!isAdmins) return reply('*📛 𝑶𝒏𝒍𝒚 𝒈𝒓𝒐𝒖𝒑 𝒂𝒅𝒎𝒊𝒏𝒔 𝒐𝒓 𝒕𝒉𝒆 𝒐𝒘𝒏𝒆𝒓 𝒄𝒂𝒏 𝒖𝒔𝒆 𝒕𝒉𝒊𝒔 𝒄𝒐𝒎𝒎𝒂𝒏𝒅.*');
...
cmd({
  ...
  try {
    ...
    reply("Sᴜᴄᴄᴇssғᴜʟʟʏ ʟᴇғᴛ ᴛʜᴇ ɢʀᴏᴜᴘ.🙂💔");
  } catch (error) {
    console.error(error);
    reply("Fᴀɪʟᴇᴅ ᴛᴏ ʟᴇᴀᴠᴇ ᴛʜᴇ ɢʀᴏᴜᴘ.🤦🏻🥲");
  }
});
//--------------------------------------------
//  MUGHAL-MD 
...
cmd({
    ...
    try {
        ...
        if (!isGroup) return reply("𝕋𝕙𝕚𝕤 𝕗𝕖𝕒𝕥𝕦𝕣𝕖 𝕚𝕤 𝕠𝕟𝕝𝕪 𝕗𝕠𝕣 𝕘𝕣𝕠𝕦𝕠❗");
        ...
        if (!isBotAdmins) return reply("𝙿𝚕𝚎𝚊𝚜𝚎 𝚙𝚛𝚘𝚟𝚒𝚍𝚎 𝚖𝚎 𝚊𝚍𝚖𝚒𝚗 𝚛𝚘𝚕𝚎🫩");

        // Check if the sender is an admin
        const isAdmins = isGroup ? groupAdmins.some(admin => admin.id === sender) : false;
        if (!isAdmins) return reply("𝙿𝚕𝚎𝚊𝚜𝚎 𝚙𝚛𝚘𝚟𝚒𝚍𝚎 𝚖𝚎 𝚊𝚍𝚖𝚒𝚗 𝚛𝚘𝚕𝚎🫩");
