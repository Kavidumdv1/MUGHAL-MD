...
cmd({
    ...
    try {
        ...
        if (!isBotAdmins) return reply("❌ Bot must be admin first to promote members.");
...
cmd({
    ...
    try {
        if (!isGroup) return reply("❌ This command can only be used in groups.");
        if (!isAdmins && !isOwner) return reply("❌ Only group admins or the owner can use this command.");
        if (!isBotAdmins) return reply("❌ Bot must be admin to demote members.");
...
async (conn, mek, m, { from, isGroup, senderNumber, isAdmins, isBotAdmins, reply }) => {
    try {
        ...
        reply("*gʀσᴜρ мᴜтє∂ sᴜᴄᴄєѕѕfᴜℓℓу🔐BY 𝚳𝐔𝐆𝚮𝚫𝐋 𝚳𝐃 𝐒𝐔𝚸𝚸𝚯𝚪𝚻 ⚜*");
    } catch (e) {
...
async (conn, mek, m, { from, isGroup, senderNumber, isAdmins, isBotAdmins, reply }) => {
    try {
        ...
        reply("*gʀσᴜρ ᴜɴмυтє∂ sᴜᴄᴄєѕѕfᴜℓℓу🔓 BY 𝚳𝐔𝐆𝚮𝚫𝐋 𝚳𝐃 𝐒𝐔𝚸𝚸𝚯𝚪𝚻 ⚜*");
    } catch (e) {
